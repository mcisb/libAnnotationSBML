...
public class PubChemTerm extends OntologyTerm
{
	...
	protected void doInitialise() throws Exception
	{
		...
		final boolean isCompound = ontology.getName().equals( Ontology.PUBCHEM_COMPOUND );
		final String ID = isCompound ? "compound/CID/" : "substance/SID/"; //$NON-NLS-1$ //$NON-NLS-2$
		...
		if( isCompound )
		{
			...
			try ( final BufferedReader reader = new BufferedReader( new InputStreamReader( url.openStream() ) ) )
			{
				...
				final String inchiId = org.apache.commons.lang3.StringUtils.join( Arrays.copyOfRange( tokens, INCHI, tokens.length ), SEPARATOR ).replaceAll( QUOTES, EMPTY_STRING );
		...
		final URL synonymsUrl = new URL( "https://pubchem.ncbi.nlm.nih.gov/rest/pug/" + ID + id + "/synonyms/txt" ); //$NON-NLS-1$ //$NON-NLS-2$
